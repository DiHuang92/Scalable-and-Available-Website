...
public class SessionServelet extends HttpServlet{
	...
	public static final String N_F_FILE = "/NF_info.txt";
...
//	public static final int COOKIE_AGE = 60 * 5;
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) 
		...
		if(param.equals("Replace")) {
			...
			response.setHeader("Cache-Control", "private, no-store, no-cache, must-revalidate");
			RequestDispatcher dispatcher = request.getRequestDispatcher(LOG_OUT);
			dispatcher.forward(request, response);			
	...
	/*
	 * Restore F-resilient information from local file
	 */
	private boolean restoreServerResilientInfo(){
		
		try (BufferedReader br = new BufferedReader(new FileReader(N_F_FILE)))
		{
			String instanceNumber = br.readLine();
			String resilientNumber = br.readLine();
			Utils.N = Integer.parseInt(instanceNumber);
			Utils.F = Integer.parseInt(resilientNumber);
			Utils.W = 2*Utils.F + 1;
			Utils.R = Utils.F + 1;
			Utils.WQ = Utils.R;
			System.out.println("Server Resilient Info already be read, N:"+Utils.N+" , F: "+Utils.F);
			return true;
